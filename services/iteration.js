...
⚠ **JSON 引号纪律(关键!)**:
   你输出的是 JSON,字符串值的 **首尾用半角双引号** \`"\`。
   字符串内部如果要引用 词组 / 标识 / 用户原话,**必须**用以下之一,绝不要用 \`"\`:
     - 中文场景用全角 「」 或 『』(推荐)
     - 英文场景用单引号 '
   反例(会让 JSON 崩):"summary": "协议要求"同时涉及设计思维"过窄"
   正例:            "summary": "协议要求「同时涉及设计思维」过窄"
                    "summary": "AI exclude reason was 'no LLM mention'"
